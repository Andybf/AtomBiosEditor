...
-(void)viewDidLoad {
    ...
    //Table Configuration
    tableView = [[AtomTable alloc] initWithFrame: NSMakeRect(0, 0, 420, 356)];
...
-(void)TurnOnSection : (struct ATOM_BASE_TABLE *)atmtable {
    atomTable = *atmtable;
...
- (IBAction)tableSelectorChanged:(id)sender {
    ...
    if ( [ [[ _selectorTable selectedItem] title] isEqual: @"select.."] ) {
        ...
        [self initTableTabInfo: 1 : &tempAtomTable : _radioHexadecimal];
    } else if ( [ [[ _selectorTable selectedItem] title] isEqual: @"Command Tables"] ) {
        [ _selectorTable setTitle: @"Command Tables"];
        [self initTableTabInfo: 2 : &tempAtomTable : _radioDecimal];
    } else {
        exit(5);
    }
}
...
- (IBAction)RadioHexChanged:(id)sender {
    if (_radioHexadecimal.state == 1 ) {
        ...
        [self initTableTabInfo: self.selectorTable.indexOfSelectedItem : &(atomTable) : _radioHexadecimal];
    }
}
